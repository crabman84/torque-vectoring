...
extern const char CombinedTractionLaunchControl_LibIdent[];
const char CombinedTractionLaunchControl_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_CombinedTractionLaunchControl *rtm)
{
...
static void
CombinedTractionLaunchControl_DeclQuants (void *MP)
{
    rtModel_CombinedTractionLaunchControl *rtm = (rtModel_CombinedTractionLaunchControl *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(CombinedTractionLaunchControl_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(CombinedTractionLaunchControl_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'\n",
		    Modelname, CombinedTractionLaunchControl_DictDefines[i].Name);
	}
...
/*
 * CombinedTractionLaunchControl_SetParams() will be invoked indirectly by the generated
 * model C code each time CombinedTractionLaunchControl_New() is called.
 */
void
CombinedTractionLaunchControl_SetParams (rtModel_CombinedTractionLaunchControl *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
assignCfgIF (struct tPowerTrainXWD_CfgIF *CfgIF, void *MP)
{
    ...
    ExternalInputs_CombinedTractionLaunchControl  *in  = (ExternalInputs_CombinedTractionLaunchControl *) rtmGetU(rtm);
...
static void *
CombinedTractionLaunchControl_New (struct tInfos *Inf, struct tPowerTrainXWD_CfgIF *CfgIF, const char *KindKey)
{
    ...
    MatSupp_ResetQuants(CombinedTractionLaunchControl_DictDefines);
...
static void
CombinedTractionLaunchControl_Delete (void *MP)
{
    rtModel_CombinedTractionLaunchControl *rtm = (rtModel_CombinedTractionLaunchControl *)MP;
...
static int
CombinedTractionLaunchControl_Calc (void *MP, struct tPowerTrainXWD_IF  *IF, double dt)
{
    rtModel_CombinedTractionLaunchControl *rtm = (rtModel_CombinedTractionLaunchControl *)MP;
    int osCount=0;
    ExternalInputs_CombinedTractionLaunchControl  *in  = (ExternalInputs_CombinedTractionLaunchControl *) rtmGetU(rtm);
    ExternalOutputs_CombinedTractionLaunchControl *out = (ExternalOutputs_CombinedTractionLaunchControl *)rtmGetY(rtm);
...
int
CombinedTractionLaunchControl_Register (void)
{
    tModelClassDescr m;
    ...
    m.PowerTrainXWD.DeclQuants		= CombinedTractionLaunchControl_DeclQuants;
    m.PowerTrainXWD.New			= CombinedTractionLaunchControl_New;
    m.PowerTrainXWD.Calc		= CombinedTractionLaunchControl_Calc;
    m.PowerTrainXWD.Delete		= CombinedTractionLaunchControl_Delete;
