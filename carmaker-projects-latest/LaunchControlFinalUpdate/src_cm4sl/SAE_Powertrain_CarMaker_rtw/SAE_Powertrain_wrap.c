...
extern const char SAE_Powertrain_LibIdent[];
const char SAE_Powertrain_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_SAE_Powertrain *rtm)
{
...
static void
SAE_Powertrain_DeclQuants (void *MP)
{
    rtModel_SAE_Powertrain *rtm = (rtModel_SAE_Powertrain *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(SAE_Powertrain_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(SAE_Powertrain_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'\n",
		    Modelname, SAE_Powertrain_DictDefines[i].Name);
	}
...
/*
 * SAE_Powertrain_SetParams() will be invoked indirectly by the generated
 * model C code each time SAE_Powertrain_New() is called.
 */
void
SAE_Powertrain_SetParams (rtModel_SAE_Powertrain *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
assignCfgIF (struct tPowerTrainXWD_CfgIF *CfgIF, void *MP)
{
    ...
    ExternalInputs_SAE_Powertrain  *in  = (ExternalInputs_SAE_Powertrain *) rtmGetU(rtm);
...
static void *
SAE_Powertrain_New (struct tInfos *Inf, struct tPowerTrainXWD_CfgIF *CfgIF, const char *KindKey)
{
    rtModel_SAE_Powertrain *rtm;
    ...
    MatSupp_ResetQuants(SAE_Powertrain_DictDefines);
...
static int
SAE_Powertrain_Calc (void *MP, struct tPowerTrainXWD_IF  *IF, double dt)
{
    rtModel_SAE_Powertrain *rtm = (rtModel_SAE_Powertrain *)MP;
    int osCount=0;
    ExternalInputs_SAE_Powertrain  *in  = (ExternalInputs_SAE_Powertrain *) rtmGetU(rtm);
    ExternalOutputs_SAE_Powertrain *out = (ExternalOutputs_SAE_Powertrain *)rtmGetY(rtm);
...
SAE_Powertrain_Register (void)
{
    ...
    m.PowerTrainXWD.DeclQuants		= SAE_Powertrain_DeclQuants;
    m.PowerTrainXWD.New			= SAE_Powertrain_New;
    m.PowerTrainXWD.Calc		= SAE_Powertrain_Calc;
    m.PowerTrainXWD.Delete		= SAE_Powertrain_Delete;
