...
extern const char RuleBasedTractionControl_LibIdent[];
const char RuleBasedTractionControl_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_RuleBasedTractionControl *rtm)
{
...
static void
RuleBasedTractionControl_DeclQuants (void *MP)
{
    rtModel_RuleBasedTractionControl *rtm = (rtModel_RuleBasedTractionControl *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(RuleBasedTractionControl_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(RuleBasedTractionControl_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'\n",
		    Modelname, RuleBasedTractionControl_DictDefines[i].Name);
	}
...
/*
 * RuleBasedTractionControl_SetParams() will be invoked indirectly by the generated
 * model C code each time RuleBasedTractionControl_New() is called.
 */
void
RuleBasedTractionControl_SetParams (rtModel_RuleBasedTractionControl *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
assignCfgIF (struct tPowerTrainXWD_CfgIF *CfgIF, void *MP)
{
    ...
    ExternalInputs_RuleBasedTractionControl  *in  = (ExternalInputs_RuleBasedTractionControl *) rtmGetU(rtm);
...
static void *
RuleBasedTractionControl_New (struct tInfos *Inf, struct tPowerTrainXWD_CfgIF *CfgIF, const char *KindKey)
{
    rtModel_RuleBasedTractionControl *rtm;
    ...
    MatSupp_ResetQuants(RuleBasedTractionControl_DictDefines);
...
static void
RuleBasedTractionControl_Delete (void *MP)
{
    rtModel_RuleBasedTractionControl *rtm = (rtModel_RuleBasedTractionControl *)MP;
...
static int
RuleBasedTractionControl_Calc (void *MP, struct tPowerTrainXWD_IF  *IF, double dt)
{
    rtModel_RuleBasedTractionControl *rtm = (rtModel_RuleBasedTractionControl *)MP;
    int osCount=0;
    ExternalInputs_RuleBasedTractionControl  *in  = (ExternalInputs_RuleBasedTractionControl *) rtmGetU(rtm);
    ExternalOutputs_RuleBasedTractionControl *out = (ExternalOutputs_RuleBasedTractionControl *)rtmGetY(rtm);
...
int
RuleBasedTractionControl_Register (void)
{
    tModelClassDescr m;
    ...
    m.PowerTrainXWD.DeclQuants		= RuleBasedTractionControl_DeclQuants;
    m.PowerTrainXWD.New			= RuleBasedTractionControl_New;
    m.PowerTrainXWD.Calc		= RuleBasedTractionControl_Calc;
    m.PowerTrainXWD.Delete		= RuleBasedTractionControl_Delete;
