...
extern const char CalebsLaunchControl_LibIdent[];
const char CalebsLaunchControl_LibIdent[] = "(@@)" QUOTE(MODEL) " " ARCH " 1.0 " BUILDDATE;
...
static void
DoOneStep (rtModel_CalebsLaunchControl *rtm)
{
...
static void
CalebsLaunchControl_DeclQuants (void *MP)
{
    rtModel_CalebsLaunchControl *rtm = (rtModel_CalebsLaunchControl *)MP;
    int i;
	...
	/* Remember body frames defined in this model for later registration. */
        MdlBdyFrame_Add(CalebsLaunchControl_BdyFrameDefines);

	/* Define dict entries for non-dynamically allocated variables. */
	if ((i = MatSupp_DeclQuants(CalebsLaunchControl_DictDefines)) >= 0) {
	    LogErrF(EC_Init, "Model '%s': Error defining quantity '%s'\n",
		    Modelname, CalebsLaunchControl_DictDefines[i].Name);
	}
...
/*
 * CalebsLaunchControl_SetParams() will be invoked indirectly by the generated
 * model C code each time CalebsLaunchControl_New() is called.
 */
void
CalebsLaunchControl_SetParams (rtModel_CalebsLaunchControl *rtm, struct tMatSuppTunables *tuns,
		   struct tInfos *Inf)
{
    /*Log("%s_SetParams()\n", Modelname);*/
...
assignCfgIF (struct tPowerTrainXWD_CfgIF *CfgIF, void *MP)
{
    ...
    ExternalInputs_CalebsLaunchControl  *in  = (ExternalInputs_CalebsLaunchControl *) rtmGetU(rtm);
...
static void *
CalebsLaunchControl_New (struct tInfos *Inf, struct tPowerTrainXWD_CfgIF *CfgIF, const char *KindKey)
{
    rtModel_CalebsLaunchControl *rtm;
    ...
    MatSupp_ResetQuants(CalebsLaunchControl_DictDefines);
...
static void
CalebsLaunchControl_Delete (void *MP)
{
    rtModel_CalebsLaunchControl *rtm = (rtModel_CalebsLaunchControl *)MP;
...
static int
CalebsLaunchControl_Calc (void *MP, struct tPowerTrainXWD_IF  *IF, double dt)
{
    rtModel_CalebsLaunchControl *rtm = (rtModel_CalebsLaunchControl *)MP;
    int osCount=0;
    ExternalInputs_CalebsLaunchControl  *in  = (ExternalInputs_CalebsLaunchControl *) rtmGetU(rtm);
    ExternalOutputs_CalebsLaunchControl *out = (ExternalOutputs_CalebsLaunchControl *)rtmGetY(rtm);
...
int
CalebsLaunchControl_Register (void)
{
    tModelClassDescr m;
    ...
    m.PowerTrainXWD.DeclQuants		= CalebsLaunchControl_DeclQuants;
    m.PowerTrainXWD.New			= CalebsLaunchControl_New;
    m.PowerTrainXWD.Calc		= CalebsLaunchControl_Calc;
    m.PowerTrainXWD.Delete		= CalebsLaunchControl_Delete;
